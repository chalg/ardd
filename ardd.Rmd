...
Estimated population can be sourced from  [here](https://www.abs.gov.au/statistics/people/population/national-state-and-territory-population/latest-release)
Scroll down to 'States and territories' Annual population change.
...
# Review count of fatalities by year, excluding 2017
hist3 <- fatal %>% 
ggplot(aes(x = year)) +
  scale_x_continuous(limits = c(1989, 2023), breaks = seq(1989, 2023, 3)) +
  geom_histogram(binwidth = 1, fill = my_colours[1], alpha = 0.70) +
  labs(x = "Year", y = "Count",
...
box2 <- fatal %>% filter(!gender == "Unknown",
  ...
  stat_summary(fun.y = mean, geom = 'point', shape = 4) +
  labs(x = "Gender", y = "Age",
       title = 'Australian Road Fatalities (1989-2023)') +
  theme(plot.title = element_text(size = 12, face = "bold"),
        plot.subtitle = element_text(size = 10, color = "gray20"),
        axis.title = element_text(size = 10), legend.position = "bottom",
        axis.text.x = element_text(angle = 0, size = 9),
...
fatal_unique %>%
  ...
  scale_x_continuous(breaks = seq(1989, 2022, 1)) +
...
fatal %>% filter(!gender == "Unspecified",
  ...
  labs(x = "Age", y = "Number of Fatalities",
       title = 'Australian Road Fatalities by Road User, Age and Gender (1989-2023)',
       colour = "Road User",
       caption = "Source: @GrantChalmers | Australian Road Deaths Database (ARDD)") +
  theme(plot.title = element_text(size = 12, face = "bold"),
        plot.subtitle = element_text(size = 10, color = "gray20"),
...
fatal_unique %>% 
  ...
  geom_line(col = "purple", size = 1) +
  geom_smooth(method = 'loess', color = 'green') +
  scale_x_continuous(breaks = seq(1989, 2022, 3), limits = c(1989, 2022)) + 
  facet_wrap( ~ state, scales = "free_y", ncol = 2) +
  labs(x = "Year", y = "Number of Fatalities",
       title = 'Australian Road Fatalities by State (1989-2023)',
       caption = "Source: @GrantChalmers | Australian Road Deaths Database (ARDD)") +
  theme(plot.title = element_text(size = 12, face = "bold"),
        plot.subtitle = element_text(size = 10, color = "gray20"),
        axis.title = element_text(size = 10), legend.position = "right",
...
plt1 <- fatal_state_apop %>% 
  ...
  labs(x = "State", y = "Fatalities per year for every 100,000 people",
       title = 'Australian Road Fatality Rate by State (1989-2023)') +
