...
lapply(c("tidyverse", "lubridate", "purrr", "janitor", "scales", "patchwork"),
...
plt1 <- fatal_state_apop %>% 
  ...
  geom_hline(aes(yintercept = mean_fatal_rate), colour="#BB0000",
             linetype="dashed") +
  annotate("text", x = "WA", y = fatal_state_apop$mean_fatal_rate + 0.5, label = paste0("Mean == ", round(fatal_state_apop$mean_fatal_rate, 3)),
...
(Population Estimates by Statistical Area Level 2 (ASGS 2016), 2017 to 2018) [here](https://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/3218.02017-18?OpenDocument).
